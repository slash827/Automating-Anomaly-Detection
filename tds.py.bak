...
pyod has feature to automatically generate datasets with outliers, and metaod was trained 
with that so we also wanted to include those datasets as well
"""
...
def preprocess_creditcard(path=ROOT_DIR + "creditcard.csv"):
    data = pd.read_csv(path, nrows=25000)
...
def load_data(dataset_name):
    if dataset_name == 'credit-card':
        return preprocess_creditcard(ROOT_DIR + "creditcard.csv")
    if dataset_name == 'loans':
        return preprocess_loan(ROOT_DIR + 'loan_data.csv')
    if dataset_name == 'pulsars':
        return preprocess_pulsar(ROOT_DIR + 'Pulsar.csv')
...
def automate_model_selection(x, y, is_metaod_generated=False):
    ...
    if is_metaod_generated:
        ...
        print("This dataset is suitable for using metaOD! we will find the recommended models,"
              " and then try to improve them even more")
